...
object listfun {
	...
	val chars = List("a","a","a", "b", "c","c", "a")
                                                  //> chars  : List[String] = List(a, a, a, b, c, c, a)
	
	pack(chars)                               //> res6: List[List[String]] = List(List(a, a, a), List(b), List(c, c), List(a))
                                                  //| 
  def encode[T](xs: List[T]): List[(T,Int)] =
  	pack(xs) map (ys => (ys.head, ys.length)) //> encode: [T](xs: List[T])List[(T, Int)]
  	
  encode(chars)                                   //> res7: List[(String, Int)] = List((a,3), (b,1), (c,2), (a,1))
